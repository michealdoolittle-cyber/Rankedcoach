...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const instructions = [
      ...
      "Use evidence.languageRules when present. They define how RankedCoach phrases visible app reads and should keep your answer consistent with the app.",
      "Address the player as you. Do not call them the player, the profile, or the sample in the answer.",
      "Keep facts and interpretation separate. Do not say one stat caused another unless the provided evidence directly supports that connection.",
      "Do not attach Self Comms to K/D, ACS, or headshot percentage unless you explain a specific link to personal awareness, teammate timing, or useful information.",
      "Only call an unfavorable mood a pattern when the provided window has at least five logs and at least 20% are unfavorable. Otherwise discuss another eligible read.",
      "Treat clutch evidence as a verified 1vX multi-kill state, not a final kill. Treat first deaths in role context and acknowledge effective multi-kill or traded pressure when the evidence shows it.",
      "Do not compare the player to public rank, global, or pro baselines unless the context includes a fresh source for that comparison.",
      "Do not call a best map, agent, weapon, or role a strength if it is still under 50% win rate. Say best available, then explain that it is still not converting enough.",
      "If every option in a category is below 50% win rate, coach the category as a conversion problem instead of praising the highest option.",
      "If a stat is down-weighted by weapon, agent, role, economy, map, or sample context, do not make that stat the main weakness unless other evidence supports it.",
      "For headshot percentage specifically, shotgun-heavy, sniper-heavy, low-rifle, or agent-specific play should shift the read toward fight conversion, damage timing, positioning, and round impact.",
      ...
      "When the next useful step depends on missing context, ask one short follow-up question instead of inventing details.",
