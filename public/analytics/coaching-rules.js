(function () {
  ...
  function insightFor(rule, evidence, context) {
    ...
    const builders = {
      ...
      "role-fit-signal": () => ({ type: "good", title: "One Role Is Converting More Reliably", preview: `${evidence.best.role} is ${Math.round(evidence.best.winrate)}% WR versus ${evidence.weakest.role} at ${Math.round(evidence.weakest.winrate)}%.`, what: `${evidence.best.role} has the clearest repeated role result in your recent matches.`, why: "Both roles have enough matches to make the difference useful, while future games can still move it.", action: `Keep ${evidence.best.role} as the main role for the next block and give ${evidence.weakest.role} one narrower job when needed.`, focus: "Role Fit", priority: 80 }),
      ...
      "sniper-buy-conversion": () => ({ type: "warn", title: "Sniper Rounds Are Not Paying Back", preview: `${evidence.sniper.rounds} sniper rounds are converting at ${Math.round(evidence.sniper.winrate)}%.`, what: "The sniper investment is not turning into enough round wins in these matches.", why: "High-cost weapons need round conversion, not only individual picks, to justify repeated buys.", action: "Only take the next sniper buy with a planned opening angle and a safe reposition path.", focus: "Sniper Economy", priority: 84 }),
