(function () {
  ...
  const SOURCE_VERSION = "2026-07-11";
  ...
  const RULES = Object.freeze([
    ...
    { id: "tilt-performance-correlation", sourceRule: 258, category: "communication", status: "active", condition: "tilt-performance-correlation", minMatches: 6, minLogs: 4 },
  ...
  function evidenceFor(condition, context) {
    ...
    if (condition === "tilt-performance-correlation") {
      return number(logs.negativeMoodRate) >= 50 && number(overview.winrate) < 45
  ...
  function insightFor(rule, evidence, context) {
    ...
    const builders = {
      "map-early-leader": () => ({ type: "good", title: "Map Result Is Promising, Not Proven", preview: `${evidence.leader.map} is at ${Math.round(evidence.leader.winrate)}% WR across ${evidence.leader.matchesPlayed} matches.`, what: `${evidence.leader.map} is ahead of the other map samples, but the sample is still early.`, why: "Fewer than ten matches can point in a direction without proving a durable map strength.", action: `Keep one repeatable plan for ${evidence.leader.map} and recheck it after the sample reaches ten matches.`, focus: "Map Planning", priority: 72 }),
      "map-pool-conversion": () => ({ type: "warn", title: "Map Pool Needs One Stable Plan", preview: `All ${evidence.repeatedMaps.length} repeated maps are below 50% WR.`, what: "The selected map pool is losing across several maps, not only one.", why: "A broad pattern points more toward repeatable round plans than one map-specific mistake.", action: `Start with ${evidence.best.map}: use one attack default and one defense fallback for the next block.`, focus: "Map Fundamentals", priority: 93 }),
      "map-recent-improvement": () => ({ type: "good", title: "Recent Map Work Is Moving", preview: `${evidence.map.map} rose from ${Math.round(evidence.earlierRate)}% to ${Math.round(evidence.recentRate)}% WR in the recent sample.`, what: `The latest ${evidence.map.map} games are converting better than the earlier games.`, why: "The direction is positive, even if the season-long result still needs more matches.", action: `Repeat the same ${evidence.map.map} preparation for the next three games before changing it.`, focus: "Map Progress", priority: 76 }),
      "duelist-opening-duty": () => ({ type: "warn", title: "Duelist Openings Need More Presence", preview: `Opening kills appear in ${Math.round(evidence.rate)}% of rounds in this scope.`, what: "The duelist sample is not creating enough first advantages yet.", why: "Duelists can create space without getting every first kill, so this is a role question rather than a flat K/D judgment.", action: "Choose one supported entry path and ask for the enabling utility before committing.", focus: "Entry Timing", priority: 86 }),
      "initiator-assist-balance": () => ({ type: "warn", title: "Initiator Value Needs More Follow-Through", preview: `${evidence.agent.agent} is averaging ${evidence.assistsPerMatch.toFixed(1)} assists per match.`, what: "The fights are competitive, but the initiator sample is not producing enough shared fight value.", why: "Low assist volume can mean information or utility is arriving without a teammate ready to use it.", action: "Call the next piece of utility before using it, then stay close enough to follow the teammate it enables.", focus: "Utility Follow-Through", priority: 79 }),
      "agent-pool-width": () => ({ type: "warn", title: "Agent Pool Is Changing Too Often", preview: `${evidence.agentCount} agents appeared in the last 10 matches, with no pick above ${Math.round(evidence.highestShare)}%.`, what: "The recent agent pool is too spread out to build a stable read around one job.", why: "Frequent switching makes it harder to separate an agent problem from normal adaptation time.", action: "Use a two-agent pool for the next five matches and keep each agent's job simple.", focus: "Agent Pool", priority: 82 }),
      "role-fit-signal": () => ({ type: "good", title: "One Role Is Converting More Reliably", preview: `${evidence.best.role} is ${Math.round(evidence.best.winrate)}% WR versus ${evidence.weakest.role} at ${Math.round(evidence.weakest.winrate)}%.`, what: `${evidence.best.role} is the clearest repeated role result in this scope.`, why: "Both roles have enough matches to make the difference useful, while future games can still move it.", action: `Keep ${evidence.best.role} as the main role for the next block and give ${evidence.weakest.role} one narrower job when needed.`, focus: "Role Fit", priority: 80 }),
      ...
      "damage-spike-reliance": () => ({ type: "warn", title: "Damage Is Leaning On Spike Rounds", preview: `${evidence.killsPerMatch.toFixed(1)} kills per match with high round-to-round damage variation.`, what: "The kill volume is healthy, but the damage is arriving unevenly across rounds.", why: "A few large rounds can lift the total while many rounds still begin without useful pressure.", action: "Use one repeatable early damage plan so more rounds contribute before the fight becomes chaotic.", focus: "Damage Consistency", priority: 83 }),
      ...
      "sniper-buy-conversion": () => ({ type: "warn", title: "Sniper Rounds Are Not Paying Back", preview: `${evidence.sniper.rounds} sniper rounds are converting at ${Math.round(evidence.sniper.winrate)}%.`, what: "The sniper investment is not turning into enough round wins in this sample.", why: "High-cost weapons need round conversion, not only individual picks, to justify repeated buys.", action: "Only take the next sniper buy with a planned opening angle and a safe reposition path.", focus: "Sniper Economy", priority: 84 }),
      "utility-volume-conversion": () => ({ type: "warn", title: "Utility Volume Is Not Converting Yet", preview: `${evidence.castsPerRound.toFixed(1)} recorded casts per round with ${Math.round(evidence.winrate)}% match wins.`, what: "The app sees plenty of utility being used, but not enough match conversion beside it.", why: "Cast counts show volume only; without timestamps, the app cannot claim the placement or timing was wrong.", action: "Pair one called piece of utility with one teammate action and review whether that sequence wins space.", focus: "Utility Coordination", priority: 78 }),
      "team-trade-efficiency": () => ({ type: "warn", title: "Teammate Deaths Need Faster Answers", preview: `${Math.round(evidence.rate)}% of ${evidence.opportunities} teammate-death rounds were traded by you.`, what: "Too many teammate deaths are going unanswered inside the trade window.", why: "This can come from spacing or different round jobs, so it is a coordination signal rather than an aim verdict.", action: "Start the next contact one step closer to the teammate most likely to fight first.", focus: "Trade Spacing", priority: 88 }),
      "individual-output-team-gap": () => ({ type: "warn", title: "Fight Value Is Not Becoming Team Wins", preview: `${evidence.kd.toFixed(2)} K/D with ${Math.round(evidence.winrate)}% WR and ${Math.round(evidence.tradeRate)}% trades given.`, what: "Individual fights are working better than the overall match conversion.", why: "The gap points toward what happens around advantages, especially spacing and shared follow-up.", action: "After the first advantage, group with one teammate and make the next fight a trade instead of another solo duel.", focus: "Round Conversion", priority: 91 }),
      ...
      "role-aware-trades": () => ({ type: "warn", title: "Support Trades Need To Catch Up", preview: `${Math.round(evidence.given)}% trades given versus ${Math.round(evidence.received)}% trades received on ${evidence.role}.`, what: "The support-role sample is being traded more often than it returns trades for teammates.", why: "Controllers, initiators, and sentinels often play from positions where answering the first death is part of their round value.", action: "Hold the next support position close enough to answer the first teammate contact within five seconds.", focus: "Support Spacing", priority: 85 }),
      "comms-trade-correlation": () => ({ type: "warn", title: "Low-Comms Logs Match The Trade Gap", preview: `${Math.round(evidence.commsRate)}% comms-enabled logs alongside ${Math.round(evidence.tradeRate)}% trades given.`, what: "Self-reported low comms and the objective trade gap are pointing in the same direction.", why: "The app cannot hear voice chat; this uses your logs as a proxy and checks them against match timing.", action: "Before first contact, call who you are following or who can trade you.", focus: "Communication", priority: 89 }),
      "tilt-performance-correlation": () => ({ type: "warn", title: "Tilt Logs Line Up With The Losing Window", preview: `${Math.round(evidence.moodRate)}% negative-mood logs with ${Math.round(evidence.winrate)}% WR.`, what: "The logged mood pattern and the match results are both struggling in this scope.", why: "That correlation does not prove tilt caused every loss, but it is strong enough to test a reset plan.", action: "After two tilted rounds, use one breath, one useful call, and one simple position before making another adjustment.", focus: "Mental Reset", priority: 90 }),
      "comms-kast-correlation": () => ({ type: "warn", title: "Low-Comms Logs Match Lower Round Involvement", preview: `${Math.round(evidence.commsRate)}% comms-enabled logs with ${Math.round(evidence.kast)}% KAST.`, what: "The self-reported comms sample and round involvement are both below the working range.", why: "The app cannot inspect voice chat, so it treats this as a player-authored correlation, not measured call quality.", action: "Make one concise location-and-count call before each planned teammate contact.", focus: "Useful Calls", priority: 86 }),
      "logged-comms-breakdown": () => ({ type: "warn", title: "Communication Breakdowns Keep Appearing In Logs", preview: `${evidence.mentions} recent reflections mention communication problems.`, what: "Communication is repeating in the player-authored notes often enough to keep visible.", why: "This comes from your own reflections; the app is not claiming access to team voice chat.", action: "Use one short pre-round plan call, then log whether the team followed it.", focus: "Communication", priority: 77 })
    };
