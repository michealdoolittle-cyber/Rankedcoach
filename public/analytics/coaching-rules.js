(function () {
  ...
  function insightFor(rule, evidence, context) {
    ...
    const builders = {
      ...
      "tilt-performance-correlation": () => ({ type: "warn", title: "Unfavorable Mood Logs Repeat In A Losing Stretch", preview: `${Math.round(evidence.moodRate)}% of your mood logs were unfavorable while this match group won ${Math.round(evidence.winrate)}%.`, what: "Your mood logs and results are both struggling in the same period.", why: "That overlap does not prove mood caused every loss, but at 20% or more it is frequent enough to test a reset plan.", action: "After two tilted rounds, use one breath, one useful call, and one simple position before making another adjustment.", focus: "Behavior Composure", priority: 90 }),
      "comms-kast-correlation": () => ({ type: "warn", title: "Low-Comms Logs Match Lower Round Involvement", preview: `${Math.round(evidence.commsRate)}% comms-enabled logs with ${Math.round(evidence.kast)}% KAST.`, what: "Your low-comms logs line up with fewer rounds where you are part of the outcome.", why: "RankedCoach cannot inspect voice chat, so this is a link between your own logs and match stats, not measured call quality.", action: "Make one concise location-and-count call before each planned teammate contact.", focus: "Communication", priority: 86 }),
