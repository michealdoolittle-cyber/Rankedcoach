...
**Status:** New directive. Extends the existing knowledge pipeline (`docs/knowledge-pipeline.md`, `worker/knowledge-pipeline.mjs`) — does not restructure it. The private/public boundary, the Approve-then-Publish gate, original-wording enforcement, and the `CONTENT_AUTOMATION` KV layout all stay exactly as they are. Everything below is additive: a new schema field, a previously-unused ingestion path getting wired in, and new detectors/render surfaces that consume the same approval pipeline.
