...
**Status:** Shipped 2026-07-14. Placement matches now carry an explicit `isPlacementMatch` flag from Henrik v4 through RR enrichment and feed placeholders, display a neutral `Placements` badge instead of `RR unverified`, and create labeled new-season boundaries on the Lifetime rank chart. Empirical verification used two completed Competitive matches (`145aceda-cda0-47ce-a177-0eae09a9fd06` and `cfa76000-16f0-4206-848e-cf54dc8955e9`): both returned `tier.id === 0`/`Unrated` while preserving win/loss, KDA, and full round data. No fixed placement count was introduced.
