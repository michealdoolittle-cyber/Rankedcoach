...
**Status:** Ready to build. Follows a full audit of what's already automatic in the app (requested by Michael) — two parallel research passes confirmed the entire Home/Insights/Stats pipeline (Weekly Focus, Recent Improvement, Impact Role Meter, Priority Trends, Trend Groups, Recent Match Trends, Match Patterns) already recomputes fresh from live data on every sync with zero caching or staleness risk. **Don't touch that pipeline — it's already correct.** The two real gaps found are covered here. A third finding (Match Patterns and Recent Match Trends currently share the exact same season/act scope, with no actual longer/shorter-term distinction despite Michael describing them that way) was explicitly left out of this directive — it's a product decision, not a confirmed build item, and shouldn't be acted on without him confirming he wants it.
