...
**Ship status (2026-07-08):** Resolved. Same-user auth events inside the new 5-minute sleep window now skip the full account-state reload path instead of rehydrating the whole account on every short tab return.
...
**Ship status (2026-07-08):** Resolved. Bottom-nav taps and swipe page switches now close the mobile profile popover, settings menu, Ask Coach, and the listed mobile modals before page activation.
...
**Ship status (2026-07-08):** Resolved. Mobile page activation now re-runs the swipe affordance binding after tab/page containers are recreated, so Logging and Stats swipes survive leaving and returning to the page.
...
**Ship status (2026-07-08):** Resolved. The two mobile-footer crosshair selectors now require `.border-animated`, matching the rest of the border system.
...
**Ship status (2026-07-08):** Resolved. `initializeSignedInAccount()` now runs profile hydration and security hydration in parallel and no longer burns a fixed extra 360ms delay after the work is already done.
...
**Ship status (2026-07-08):** Resolved. The second step is now correctly labeled `Security`, and the existing login-init bar now shows an in-step shimmer so it still reads as active while a single async step is taking longer.
...
**Ship status (2026-07-08):** Resolved. The login-init overlay now carries a dedicated detail line under the progress bar with player-language notes for each stage and a clear callout that larger saved accounts take longer.
...
**Ship status (2026-07-08):** Resolved for the confirmed source-side gap. Cached signed-in session restores now go through `initializeSignedInAccount()` and the existing login-init overlay instead of silently hydrating in the background. Guest/demo coverage still deserves a live pass, but the missing-loader source path was the returning signed-in session restore flow in `initUserSession()`.
...
**Ship status (2026-07-08):** Still open pending live repro. No source-side fix was shipped because the reported selector path still did not reproduce cleanly in code.
...
**Ship status (2026-07-08):** Still open pending live repro. No alignment change shipped because the current source still looked correct and we did not have a fresh mobile screenshot proving the bad state.

Checked `.lens-modal-close`/`.lens-modal-header` (`app.css:17883-17921`) - `display:flex;align-items:center`, 44x44px button, used identically by all four modals named in #11 below. No misalignment found in source. Re-verify live on mobile specifically (source doesn't rule out a mobile-only override not yet found) before fixing.

### 11. Gray exit buttons (Customization menu, Impact pill, Recent Improvement, Trend/Improvement Timeline)

**Ship status (2026-07-08):** Still open for the original gray-button repro, but the confirmed theme-consistency defect is resolved. `.lens-modal-close` now follows `var(--accent)` / `var(--accent-2)` instead of a hardcoded red gradient, so every current and future theme - including the premium QA themes - keeps matched close buttons.
...
**Ship status (2026-07-08):** Resolved. Mobile bottom-nav buttons now get an immediate press state so taps read as responsive even before the page swap finishes.
