...
**Status:** Shipped in the 2026-07-16 skin-preview/layout release. Five live CC Attribution models were added after exact weapon/color checks: Prime Vandal Orange (`1`), Prime Vandal Yellow (`3`, correcting the draft's stale `2` index), Radiant Entertainment System Phantom, Arcane Sheriff, and Glitchpop Frenzy. Ambiguous Rogue/Araxys and non-permissive candidates were not added. Missing exact 3D/color-video coverage now falls back to the selected color's official static render; frame-sampled pseudo-3D remains deferred because the current requirement favors an exact honest render over an unvalidated approximation.
