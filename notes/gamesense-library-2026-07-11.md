...
**Status (2026-07-12 scroll pass):** Library pages now use one reliable vertical scroll owner per layout, and the wide tactical map no longer traps vertical touch gestures inside its horizontal scroller. Marked tactical maps render after the strategy, role, comp, and lineup sections at the bottom of each map page. The ornamental `Round Plan`, `Role Read`, and `Gunfight Plan` labels were removed, while the remaining topic and gallery headings are centered.

**Status (2026-07-12 mobile depth pass):** The Library now declares its Current Season 26 Act 4 / Patch 13.00 scope at entry. Map dossiers include fit-first tactical maps with desktop drag, mobile drag/pinch, explicit zoom controls, and switchable location/plant-hot-spot layers. Current Breeze and Split composition cards use live OP.GG Patch 13.00 samples, show the three strongest win rates among the 20 most-played comps with game counts, and explain each selectable agent; Bind is honestly marked unavailable because the current endpoint returns no ranked sample outside the active rotation. Agent details now reveal global pick-rate direction after a delayed entrance and show current map win rates over map art. Weapon entries expose global usage, selectable art no longer overlaps, `Precision Rifles` is now `Light Rifles`, and selecting the active Library nav returns to the Library landing page. Focused desktop/mobile passthrough is tracked by `testing/visual-audit/gamesense-library.test.js`.
