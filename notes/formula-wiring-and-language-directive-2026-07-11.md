...
### Shipped verification and first-cut coverage

- `testing/henrik/live-known-match.js` still confirms the known 17/22 KAST target, 77% KAST, three trade saves, CeremonyCloser x3, CeremonyFlawless x1, real multi-kills, trades, and damage spread against production Henrik responses.
- `testing/henrik/data-reflection.test.js` independently checks offsets 0/40/80. K/D/A, ACS, ADR, HS%, KAST, result, map, agent, and season agree with the direct Henrik payload for matches from three different retained seasons.
- `testing/henrik/formulas.test.js` now includes positive assertions for clutch/ace recognition, discipline, trade asymmetry, and damage variance in addition to KAST and rank-benchmark behavior.
- `public/analytics/coaching-rules.js` contains 30 source-linked entries, five from each category. Twenty are executable matchers; the remainder are explicit governance policies or blocked rules where timing, position, or voice data does not exist.
- First-cut rule numbers: Maps 1/8/20/49/50; Agents 51/55/66/85/97; Weapons 101/102/103/105/150; Utility 151/157/184/192/200; Teamwork 201/202/206/219/250; Communication 253/258/266/271/300. These were selected because their conditions can be checked from retained match, round, weapon, rank-reference, and player-authored Logging fields without inventing positional, timing, or voice evidence.
- Rule candidates enter `buildPlayerModel()` before its existing priority, dedupe, confidence, and coaching-copy polish stages. A two-match fixture produces zero matches, and the Henrik history note stays hidden for guest/non-Henrik profiles.
- A round-by-round detail editor is intentionally not part of this pass. The available data can support a future view, but manual round tags remain gated until that product surface is designed rather than being attached to rounds players cannot inspect reliably.
