...
**Status:** Shipped 2026-07-17. The Library now exposes all 29 agents in Riot's live playable roster and all 10 supported map dossiers. Existing hand-authored entries remain intact; the missing coverage is generated from Riot's current game-content descriptions and layouts plus source-linked current reference data. Riot callout coordinates are converted from the published map transform rather than placed by eye. Costs, plant shares, weapon conversion, map fit, and composition rates show an explicit unavailable state wherever no verified public sample exists; none are estimated to fill the gap.
